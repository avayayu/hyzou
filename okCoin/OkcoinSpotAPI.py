...
class OKCoinSpot:
    ...
    def getKLine(self,symbol,type,size,since):
        KLine_source="/api/v1/kline.do"
        params = 'symbol=%(symbol)s' %{'symbol':symbol}+'&type=%(type)s' %{'type':type}+'&'+'size=%(size)s' %{'size':size}+'&'+'since=%(since)s' %{'since':since}
        return httpGet(self.__url,KLine_source,params)
